...
export class UserService {
    ...
    async subscribeCountryInfo(email: string, country: string) {
        ...
        try {
            if (isSubbed.length > 0) {
                ...
                await this.subCountryModel.findOne(
                    { email },
                    (err, doc) => {
                        if (err) {
    ...
    async register(user: UserDocument) {
        ...
        const checkIfExists = await this.userModel.find({ username: user.username, email: user.email });
    ...
    async validateUser(email: string, password: string) {
        ...
        const match: boolean = await this.authService.comparePasswords(password, result.password);
        if (match) {
    ...
    async checkForCountrySubUser(email: string, country: string) {
        ...
        const result = await this.subCountryModel.find({
            email: email,
