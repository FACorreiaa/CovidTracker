...
export class DayService {
    constructor(@InjectModel('DayOne') private readonly dayModel: Model<DayOneDocument>
    ) { }
    ...
    async findDayOneByCountry(country: string) {
        ...
        return await this.dayModel.find({}).sort([['createdAt', -1]]).limit(1).exec()
    }
    ...
    async findTotalByDayOne(country: string) {
        const result = await axios.get(`${process.env.BASE_URL}/total/dayone/country/${country}`)

        return result.data
    }

    async findTotalByDayOneLive(country: string, status: string) {
        const result = await axios.get(`${process.env.BASE_URL}/dayone/country/${country}/status/${status}/live`)

        return result.data
    }

    async findTotalByDayOneAllStatus(country: string) {
        const result = await axios.get(`${process.env.BASE_URL}/dayone/country/${country}`)
        return result.data
    }
}
