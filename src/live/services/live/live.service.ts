...
export class LiveService {
    constructor(@InjectModel('Live') private readonly liveModel: Model<LiveDocument>) { }
    ...
    async findLiveDataByCountry(country: string) {
        ...
        return await this.liveModel.find({}).sort([['createdAt', -1]]).limit(1).exec()
    }

    async findLiveDataByCountryAfterDate(country: string, status: string, date: Date) {
    ...
    async findLiveData(country: string) {
        const result = await axios.get(`${process.env.BASE_URL}/live/country/${country}`)

        return result.data
    }

    async findLiveDataAfterDate(country: string, status, date: Date) {
        const result = await axios.get(`${process.env.BASE_URL}/live/country/${country}/status/${status}/date/${date}`)
        return result.data
    }
}
